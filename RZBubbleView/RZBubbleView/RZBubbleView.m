...
//默认数据配置
- (void)initData {
    self.color = self.color ? self.color : kRZBubbleViewColor;
    self.cornerRadius = self.cornerRadius > 0 ? self.cornerRadius : kRZBubbleViewCornerRadius;
    self.triangleH = self.triangleH > 0 ? self.triangleH : kRZBubbleViewTriangleH;
    self.triangleW = self.triangleW > 0 ? self.triangleW : kRZBubbleViewTriangleW;
    if (self.triangleXY < 1) {
        if (self.triangleXYScale == 0) {
            self.triangleXYScale = 0.5;
        }
        if (self.direction == TriangleDirection_Down || self.direction == TriangleDirection_Up) {
...
-(void)showInPoint:(CGPoint)point {
    ...
    if (self.timeInterval>0) {
        myTime=self.timeInterval;
    }else{
        myTime=2.0f;
    }
    dispatch_time_t delayTime = dispatch_time(DISPATCH_TIME_NOW, (int64_t)(myTime/*延迟执行时间*/ * NSEC_PER_SEC));
